...
def snmp_monitor():
	...
	if service_status(snmp_monitor_script) == 'Running':
                print "\033[33;1mweb SNMP monitor service is already running!\033[0m\n" 
        else:
                print "Starting web SNMP monitor service...." 
                cmd = 'nohup python %s  >> %s 2>&1 &' % (script, snmp_monitor_log)
                result  = os.system(cmd)
                if result == 0:
                        print '\033[32;1mweb SNMP monitor service started successfully!\n\033[0m'
