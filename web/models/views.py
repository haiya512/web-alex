...
import json
from web.backend import tri_config
...
def runCmd(request):
    ...
    cmd = "python %s/backend/multiprocessing_runCMD2.py %s '%s' '%s' %s &" % (tri_config.Working_dir,track_mark,' '.join(iplists),user_input,user_account)
    os.system(cmd)
...
def AllCommands(request):
        cmd_list = os.popen('bash %s/web/backend/command_list.sh' %tri_config.Working_dir ).read()
...
def transfer_file(request):
	...
	if option == 'SendFile':
		...
		transfer_action = "python  %s/web/backend/multiprocessing_sftp2.py %s %s -s %s %s '%s' &" %(tri_config.Working_dir,track_mark,remote_user,compress(file_list),remote_path,ip_list_to_string)
	elif option == 'GetFile':
		local_path = tri_config.Tri_sftp_recv_dir 
		remote_file = request.POST['RemotePath']
		transfer_action = "python  %s/web/backend/multiprocessing_sftp2.py %s %s -g %s '%s' &" %(tri_config.Working_dir,track_mark,remote_user,remote_file,ip_list_to_string)
...
def getDangerousCmd(request):
	dangerous_filename = '%s/web/backend/dangerous_cmd.txt' % tri_config.Working_dir
...
def baoleihost_remote(request):
    ...
    profile_file = "%s/web/logs/%s_profile" % (triaquae_path,request.user)
